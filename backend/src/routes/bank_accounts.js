...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await Bank_accountsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await Bank_accountsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await Bank_accountsDBApi.findAll(
    req.query, { currentUser }
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id','account_name','account_number',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await Bank_accountsDBApi.findBy(
    { id: req.params.id },
  );
